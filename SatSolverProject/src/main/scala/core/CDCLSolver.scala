...
class CDCLSolver extends SATSolvingAlgorithm {
  ...
  def runToComplete(lastNode: ADecisionLiteral): Unit = {
    ...
    applyUnitPropagation(lastNode.formula) match {
      case Some((f, r, unitClauses)) =>
        digestUnitPropagation(f, r, unitClauses, lastNode)
        runToComplete(lastNode)
      case None =>
    }
  ...
  private def digestUnitPropagation(f: InternalCNF,
                            r: (String, Boolean),
                            unitClauses: Set[InternalClause],
                            lastNode: ADecisionLiteral): Unit = {
    var formula = f
    ...
    if (f.conjuncts.exists(c => !c.disjuncts.exists(d => d.isActive))) {
      ...
      updateGraphAfterUnitPropagation(lastNode, res._2, res._3)
      // we need to pass on the new formula, so that we can later pick the correct next decision literal!
      formula = res._1
    }
    updateGraphAfterUnitPropagation(lastNode, r, unitClauses)
  ...
  /**
    ...
    */
  private def updateGraphAfterUnitPropagation(lastNode: ADecisionLiteral,
                                              r: (String, Boolean),
                                              unitClauses: Set[InternalClause]): Unit = {
    val newNode = NonDecisionLiteral(r._1, r._2, lastNode)
    graph.addNode(newNode)
  ...
  def runCDCL(lastNode: ADecisionLiteral): Boolean = {
    ...
    conflictVarName match {
      case Some(name) =>
        ...
        val newLastNode = applyConflictResolution(name)
        newLastNode match {
          case Some(node) => runCDCL(node)
          case None => false
        }
      case None =>
  ...
  /**
    * resolves the conflict in the graph by doing back-jumping and learning a new clause
    */
  def applyConflictResolution(conflictName: String): Option[ADecisionLiteral] = {
    val learnedClause = CDCLGraphUtils.learnClause(graph, conflictName)

    var (newLastNodeParent, newVarName, newVarValue) = CDCLGraphUtils.doBackJumping(graph, conflictName)
  ...
  private def getMostOccurringLiteral(formula: InternalCNF,
                                      possibleDecisionLiterals: Set[InternalLiteral]): _root_.util.InternalLiteral = {
